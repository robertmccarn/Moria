...
public sealed class PixelTile : IEquatable<PixelTile>
{
    ...
    public bool Equals(PixelTile? other)
    {
        ...
        for (int y = 0; y < Height; y++) for (int x = 0; x < Width; x++) if (pixels[y, x] != other.pixels[y, x]) return false;
        return true;
    }
    public override bool Equals(object? obj) => obj is PixelTile other && Equals(other);
    public override int GetHashCode() { HashCode hash = new(); for (int y = 0; y < Height; y++) for (int x = 0; x < Width; x++) hash.Add(pixels[y, x]); return hash.ToHashCode(); }
    public Bitmap ToBitmap()
    {
        Bitmap bitmap = new(Width, Height, PixelFormat.Format32bppArgb);
        for (int y = 0; y < Height; y++) for (int x = 0; x < Width; x++) bitmap.SetPixel(x, y, pixels[y, x]);
        return bitmap;
    }
}
...
public sealed class TileSet
{
    ...
    public int GetOrAddTile(PixelTile tile)
    {
        ...
        index = tiles.Count; tiles.Add(tile); indices.Add(tile, index); return index;
    }
}
...
public sealed class Sprite
{
    ...
    public Sprite(int widthInTiles, int heightInTiles, IReadOnlyList<PixelTile> tiles) { WidthInTiles = widthInTiles; HeightInTiles = heightInTiles; Tiles = tiles; }
...
public static class SnesDither
{
    public static Bitmap Dither(Bitmap source)
    {
        ...
        for (int y = 0; y < result.Height; y++) for (int x = 0; x < result.Width; x++)
        {
            ...
            Add(work, x + 1, y, er, eg, eb, 7.0 / 16); Add(work, x - 1, y + 1, er, eg, eb, 3.0 / 16); Add(work, x, y + 1, er, eg, eb, 5.0 / 16); Add(work, x + 1, y + 1, er, eg, eb, 1.0 / 16);
        }
        return result;
    }
    private static void Add(double[,,] work, int x, int y, double r, double g, double b, double weight) { if (y < 0 || y >= work.GetLength(0) || x < 0 || x >= work.GetLength(1)) return; work[y, x, 0] += r * weight; work[y, x, 1] += g * weight; work[y, x, 2] += b * weight; }
...
public sealed class TerrainGenerator
{
    ...
    {
        Palette target = new(Color.Transparent); foreach (Color color in palette.Take(15)) target.AddColor(color); PixelTile result = new();
        for (int y = 0; y < 8; y++) for (int x = 0; x < 8; x++) result.SetPixel(x, y, target.GetNearestColor(tile.GetPixel(x, y)));
        return result;
    }
}
...
public sealed class SpriteGenerator
{
    ...
    {
        Bitmap bitmap = new(width, height, PixelFormat.Format32bppArgb);
        for (int y = 0; y < height; y++) for (int x = 0; x < (width + 1) / 2; x++)
        {
            double nx = (x - width / 2.0) / (width / 2.0), ny = (y - height / 2.0) / (height / 2.0);
            bool inside = nx * nx + ny * ny < 0.78 + random.NextDouble() * 0.12; if (!inside) continue;
            Color color = y > height * .68 ? shadow : (y < height * .35 ? highlight : body); bitmap.SetPixel(x, y, color); bitmap.SetPixel(width - 1 - x, y, color);
        }
...
public sealed class SnesArtGenerator
{
    ...
    public SnesArtGenerator(int seed = 1337) { TerrainGenerator = new TerrainGenerator(seed); SpriteGenerator = new SpriteGenerator(seed ^ 0x5A17); }
